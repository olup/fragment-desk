...
export const compileFiles = async (paths: string[]) => {
  const contents = await Promise.all(paths.map((p) => readTextFile(p)));
...
// h5, h6, noframes,
// ol, p, ul, center,
// dir, hr, menu, pre   { display: block; unicode-bidi: embed; font-family: 'Roboto Mono', serif; }
